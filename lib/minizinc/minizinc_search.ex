defmodule MinizincSearch do
  ...
  def iterative(instance, iterations, step_fun) do
    {_final_instance, _final_results} =
      Enum.reduce_while(
  ...
  def bab(%{model: model, solver_opts: solver_opts} = instance, branch_fun) do
    ...
    solver_opts =
      Keyword.put(
        solver_opts,
        :solution_handler,
        find_k_handler(1, solver_opts[:solution_handler])
      )
  ...
  ##
  def destroy_var(variable_name, values, destruction_rate, offset \\ 0)
